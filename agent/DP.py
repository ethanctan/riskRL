...
class DynamicProgramming:
    ...
    def computeVfromQ(self, Q, pi):
        ...
        '''
        # TODO 1
        return Q[range(self.nStates), pi]
    ...
    def computeQfromV(self, V):
        ...
        for s in range(self.nStates):
            for a in range(self.nActions):
                expected_return = self.R[a, s] + self.discount * np.sum(self.P[a, s, :] * V)
                Q[s, a] = expected_return
        return Q
    ...
    def valueIterationStep(self, Q):
        ...
        '''
        # TODO 5
        max_Q = np.max(Q, axis=1)  # Compute the maximum Q-value for each state
        return np.transpose(self.R + self.discount * self.P @ max_Q)
    ...
    def exactPolicyEvaluation(self, pi):
        ...
        A = np.eye(self.nStates) - self.discount * P_pi
    ...
    def approxPolicyEvaluation(self, pi, tolerance=0.01):
        ...
        while epsilon > tolerance:
            V_new = np.copy(V)
            for s in range(self.nStates):
                R_pi_s = self.R[pi[s], s]
                P_pi_s = self.P[pi[s], s, :]
                V_new[s] = R_pi_s + self.discount * np.sum(P_pi_s * V)
    ...
    def policyIterationStep(self, pi, exact):
        ...
        # TODO 9
        if exact:
            return self.extractMaxPifromV(self.exactPolicyEvaluation(pi))
        else:
